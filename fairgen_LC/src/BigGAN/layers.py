...
class DBlock(nn.Module):
    def __init__(self, in_channels, out_channels, which_conv=SNConv2d, wide=True,
                 preactivation=False, activation=None, downsample=None, layerNorm_D=False):
    ...
    def forward(self, x):
        if self.preactivation:
            ...
            if self.layerNorm_D:
                ...
                h = F.relu(F.layer_norm(x, normalized_shape))
            else:
                h = F.relu(x)
        else:
            h = x
        ...
        if self.layerNorm_D:
            ...
            h = self.conv2(self.activation(F.layer_norm(h, normalized_shape)))
        else:
