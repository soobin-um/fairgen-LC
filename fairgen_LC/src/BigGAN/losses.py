...
# wgan loss adapted for LeCam regularization
def loss_wgan_dis(dis_fake, dis_real, ema=None, it=None):
    if ema is not None:
...
def lecam_reg(dis_real, dis_fake, ema, config):
    if config['single_anchor']:
        reg = torch.mean(F.relu(dis_real + ema.D_real).pow(2)) + \
            torch.mean(F.relu(ema.D_real - dis_fake).pow(2))
    elif config['one_sided_reg']:
        if config['one_sided_reg_real']:
            reg = torch.mean(F.relu(dis_real - ema.D_fake).pow(2))
        elif config['one_sided_reg_fake']:
            reg = torch.mean(F.relu(ema.D_real - dis_fake).pow(2))
    else:
        reg = torch.mean(F.relu(dis_real - ema.D_fake).pow(2)) + \
            torch.mean(F.relu(ema.D_real - dis_fake).pow(2))
...
# Hinge loss adapted for LeCam regularization
