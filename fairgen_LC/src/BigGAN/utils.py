...
def prepare_parser():
    ...
    parser.add_argument(
        '--v_loss_type', type=str, default='hinge_dis_linear_gen',
        help='use what type of loss'
    )
    ...
    parser.add_argument('--layerNorm_D_v', action='store_true', default=False, help='Use layerNorm in D_v? (default: %(default)s)')
    ...
    parser.add_argument(
        '--LC', type=float, default=0,
        help='LeCam Regularization for GANs (default: %(default)s)')
    parser.add_argument(
        '--adaptive_LC', action='store_true', default=False,
        help='Use an adaptive LC regularization? (default: %(default)s)')
    parser.add_argument(
        '--LC_mu', type=float, default=0,
        help='Weight used for an adaptive LeCam Regularization (default: %(default)s)')
    parser.add_argument(
        '--ema_losses_decay', type=float, default=0.99,
        help='Decay rate for ema_losses (default: %(default)s)')
    parser.add_argument(
        '--ema_losses_start', type=int, default=1000,
        help='When to start updating the EMA of discriminator outputs (default: %(default)s)')
    parser.add_argument(
        '--single_anchor', action='store_true', default=False,
        help='Use only one anchor for LC regularization? (default: %(default)s)')
    parser.add_argument(
        '--one_sided_reg', action='store_true', default=False,
        help='Use one-sided LC regularization? (default: %(default)s)')
    parser.add_argument(
        '--one_sided_reg_real', action='store_true', default=False,
        help='Enable regularization on real samples? (default: %(default)s)')
    parser.add_argument(
        '--one_sided_reg_fake', action='store_true', default=False,
        help='Enable regularization on fake samples? (default: %(default)s)')
    ...
    parser.add_argument(
        ...
        ' or None (default: %(default)s)')
    parser.add_argument(
        '--D_v_param', type=str, default='SN',
        help='Parameterization style to use for D_v, spectral norm (SN) or SVD (SVD)'
        ' or None (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_ch', type=int, default=64,
        help='Channel multiplier for D_v (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_depth', type=int, default=1,
        help='Number of resblocks per stage in D_v? (default: %(default)s)')
    parser.add_argument(
        '--D_thin', action='store_false', dest='D_wide', default=True,
        help='Use the SN-GAN channel pattern for D? (default: %(default)s)')
    parser.add_argument(
        '--D_v_thin', action='store_false', dest='D_wide', default=True,
        help='Use the SN-GAN channel pattern for D_v? (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_nl', type=str, default='relu',
        help='Activation function for D_v (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        '(default: %(default)s)')
    parser.add_argument(
        '--D_v_attn', type=str, default='64',
        help='What resolutions to use attention on for D_v (underscore separated) '
        '(default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_init', type=str, default='ortho',
        help='Init style to use for D_v(default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_lr', type=float, default=2e-4,
        help='Learning rate to use for validation Discriminator (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_B1', type=float, default=0.0,
        help='Beta1 to use for validation Discriminator (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_B2', type=float, default=0.999,
        help='Beta2 to use for Discriminator (default: %(default)s)')

    ### Batch size, parallel, and precision stuff ###
    ################### modified by sum: lambda_ ###############
    parser.add_argument(
        '--lambda_', type=float, default=0.5,
        help='Lambda for the validation network factor (default: %(default)s)')
    parser.add_argument(
        '--batch_size', type=int, default=64,
        help='Default overall batchsize (default: %(default)s)')
    parser.add_argument(
        '--v_batch_size', type=int, default=64,
        help='Default overall batchsize (default: %(default)s)')
    parser.add_argument(
        '--G_batch_size', type=int, default=0,
        help='Batch size to use for G; if 0, same as D or D_v (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        '(default: %(default)s)')
    parser.add_argument(
        '--num_D_v_steps', type=int, default=2,
        help='Number of D_v steps per G step (default: %(default)s)')
    parser.add_argument(
        '--num_D_v_accumulations', type=int, default=1,
        help='Number of passes to accumulate D_v''s gradients over '
        '(default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_fp16', action='store_true', default=False,
        help='Train with half-precision in D_v? (default: %(default)s)')
    parser.add_argument(
        '--D_mixed_precision', action='store_true', default=False,
        help='Train with half-precision activations but fp32 params in D? '
        '(default: %(default)s)')
    parser.add_argument(
        '--D_v_mixed_precision', action='store_true', default=False,
        help='Train with half-precision activations but fp32 params in D_v? '
        '(default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Calculate intra FID? (default: %(default)s)')
    parser.add_argument(
        '--FID_off', action='store_true', default=False,
        help='Skip calculating FID when computing intra FID? (default: %(default)s)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--D_v_ortho', type=float, default=0.0,
        help='Modified ortho reg coefficient in D_v (default: %(default)s)')
    parser.add_argument(
...
def add_sample_parser(parser):
    ...
    parser.add_argument(
        ...
        '(default: %(default)s)')
    parser.add_argument(
        '--sample_num_group_npz', type=int, default=10000,
        help='Number of images for each group to sample when sampling NPZs '
        '(default: %(default)s)')
    parser.add_argument(
        '--sample_num_sets', type=int, default=10,
        help='Number of sets of images for evaluation '
        '(default: %(default)s)')
    parser.add_argument(
...
################# modified by sum: validation as input ########################
def get_data_loaders(validation, config, dataset, data_root=None, augment=False, 
    ...
    loader_kwargs = {'num_workers': config['num_workers'], 'pin_memory': pin_memory,
...
def save_ema_losses_to_state_dict(ema_losses, state_dict):
    state_dict['ema_losses_D_real'] = ema_losses.D_real
    state_dict['ema_losses_D_fake'] = ema_losses.D_fake
    
def load_ema_losses_from_state_dict(ema_losses, state_dict):
    ema_losses.D_real = state_dict['ema_losses_D_real']
    ema_losses.D_fake = state_dict['ema_losses_D_fake']
...
def save_weights(G, D, D_v, state_dict, weights_root, experiment_name,
    ...
    torch.save(D_v.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_v', name_suffix])))
    torch.save(D_v.optim.state_dict(),
               '%s/%s.pth' % (root, join_strings('_', ['D_v_optim', name_suffix])))
...
def load_weights(G, D, D_v, state_dict, weights_root, experiment_name,
    ...
    if D_v is not None:
        D_v.load_state_dict(
            torch.load('%s/%s.pth' %
                       (root, join_strings('_', ['D_v', name_suffix]))),
            strict=strict)
        if load_optim:
            D_v.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['D_v_optim', name_suffix]))))
    # Load state dict
...
def sample_sheet(G, classes_per_sheet, num_classes, samples_per_class, parallel,
                 samples_root, experiment_name, folder_number, config, z_=None):
...
def interp_sheet(G, num_per_sheet, num_midpoints, num_classes, parallel,
                 samples_root, experiment_name, folder_number, config, sheet_number=0,
