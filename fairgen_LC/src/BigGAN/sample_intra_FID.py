...
def run(config):
    ...
    utils.load_weights(G if not (config['use_ema']) else None, None, None, state_dict, weights_root,
...
#                        strict=False, load_optim=False)
    ...
    for k in range(config['sample_num_sets']):
        ...
        print('For computing intra-FID, sampling %s images for each group and saving them to npz...' % config['sample_num_group_npz'])
        ...
        while(z1_count < config['sample_num_group_npz'] or z0_count < config['sample_num_group_npz']):
            x, y = [], []
            for i in trange(int(np.ceil(config['sample_num_npz'] / float(G_batch_size)))):
                with torch.no_grad():
                    images, labels = sample()
            ...
            if z1_count < config['sample_num_group_npz']:
                z1_samples.append(x[preds==1])
                z1_labels.append(y[preds==1])
                z1_count += len(x[preds==1])

            elif z0_count < config['sample_num_group_npz']:
                z0_samples.append(x[preds==0])
                z0_labels.append(y[preds==0])
                z0_count += len(x[preds==0])

        z1_samples = np.concatenate(z1_samples)[:config['sample_num_group_npz']]
        z1_labels = np.concatenate(z1_labels)[:config['sample_num_group_npz']]

        z0_samples = np.concatenate(z0_samples)[:config['sample_num_group_npz']]
        z0_labels = np.concatenate(z0_labels)[:config['sample_num_group_npz']]
    ...
    l2_db = np.zeros(config['sample_num_sets'])
    l1_db = np.zeros(config['sample_num_sets'])
    kl_db = np.zeros(config['sample_num_sets'])
    FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    z1_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    z0_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    ...
    # number of classes
    probs_db = np.zeros((config['sample_num_sets'], config['sample_num_npz'], n_classes))
    for i in range(config['sample_num_sets']):
        ...
        probs_db[i] = probs
        print('fair_disc and FID for iter {} is: l2:{}, l1:{}, kl:{}, FID: {}, z1_FID: {}, z0_FID: {}, '
              .format(i, l2, l1, kl, FID, z1_FID, z0_FID)) # modified by sum
    metrics['l2'] = l2_db
    metrics['l1'] = l1_db
    metrics['kl'] = kl_db
    ...
    print('FID_db =', FID_db) # modified by sum
    print('z1_FID_db =', z1_FID_db) # modified by sum
    print('z0_FID_db =', z0_FID_db) # modified by sum
    print('fairness_db =', l2_db)
    
    test_log.log(epoch=int(state_dict['epoch']), itr=int(state_dict['itr']),
                 FID=float(FID_db.mean().item()), 
                 Z1_FID=float(z1_FID_db.mean().item()),
                 Z0_FID=float(z0_FID_db.mean().item()),
                 FAIR=float(l2_db.mean().item()),
                 L1_FAIR=float(l1_db.mean().item()),
                 KL_FAIR=float(kl_db.mean().item()))
