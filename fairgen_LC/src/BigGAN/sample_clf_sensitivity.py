...
def run(config):
    ...
    if config['config_from_name']:
        utils.load_weights(None, None, None, state_dict, config['weights_root'],
    ...
    utils.load_weights(G if not (config['use_ema']) else None, None, None, state_dict, weights_root, experiment_name, config['load_weights'], G if config['ema'] and config['use_ema'] else None,
        strict=False, load_optim=False)
    ...
    metrics = {'l2': 0, 'l1': 0, 'kl': 0, 'l2_poor': 0, 'l1_poor': 0, 'kl_poor': 0}
    l2_db = np.zeros(10)
    l1_db = np.zeros(10)
    kl_db = np.zeros(10)
    l2_poor_db = np.zeros(10)
    l1_poor_db = np.zeros(10)
    kl_poor_db = np.zeros(10)
    FID_db = np.zeros(10) # modified by sum

    # output file
    fname = '%s/%s/FD_FID_from_samples_%s_%s_%s.p' % (
            config['samples_root'], experiment_name, config['mode'],
            'ema' if config['use_ema'] else '',
    ...
    probs_poor_fname = 'clf_probs_poor_%s_%s_%s.npy' % (config['mode'],
            'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train')
    ...
    if not config['multi']:
        ...
        # multi-attribute
        print('Pre-loading pre-trained multi-attribute classifier...')
        poor_clf_state_dict = torch.load(MULTI_CLF_PATH_celeba)['state_dict']
        clf_classes = 4
    # load attribute classifier here
    ...
    probs_poor_db = np.zeros((10, 10000, n_classes))
    for i in range(10):
        ...
        preds_poor, probs_poor = classify_examples(poor_clf, npz_filename)
        l2_poor, l1_poor, kl_poor = utils.fairness_discrepancy(preds_poor, clf_classes)
        ...
        l2_poor_db[i] = l2_poor
        l1_poor_db[i] = l1_poor
        kl_poor_db[i] = kl_poor
        
        FID_db[i] = FID # modified by sum
        probs_db[i] = probs
        probs_poor_db[i] = probs_poor
        print('fair_disc and FID for iter {} is: l2:{}, l1:{}, kl:{}, FID: {}'.format(i, l2, l1, kl, FID)) # modified by sum
        print('fair_disc (poor) and FID for iter {} is: l2:{}, l1:{}, kl:{}, FID: {}'.format(i, l2_poor, l1_poor, kl_poor, FID)) # modified by sum
    metrics['l2'] = l2_db
    metrics['l1'] = l1_db
    metrics['kl'] = kl_db
    metrics['l2_poor'] = l2_poor_db
    metrics['l1_poor'] = l1_poor_db
    metrics['kl_poor'] = kl_poor_db
    ...
    np.save(os.path.join(config['samples_root'], experiment_name, probs_poor_fname), probs_poor_db)
