...
def run(config):
    ...
    for k in range(config['sample_num_sets']):
        ...
        while(z00_count < config['sample_num_group_npz'] or z01_count < config['sample_num_group_npz'] 
              or z10_count < config['sample_num_group_npz'] or z11_count < config['sample_num_group_npz']):
            x, y = [], []
            for i in trange(int(np.ceil(config['sample_num_npz'] / float(G_batch_size)))):
                with torch.no_grad():
                    images, labels = sample()
            ...
            if z00_count < config['sample_num_group_npz']:
                z00_samples.append(x[preds==0])
                z00_labels.append(y[preds==0])
                z00_count += len(x[preds==0])

            elif z01_count < config['sample_num_group_npz']:
                z01_samples.append(x[preds==1])
                z01_labels.append(y[preds==1])
                z01_count += len(x[preds==1])

            elif z10_count < config['sample_num_group_npz']:
                z10_samples.append(x[preds==2])
                z10_labels.append(y[preds==2])
                z10_count += len(x[preds==2])

            elif z11_count < config['sample_num_group_npz']:
                z11_samples.append(x[preds==3])
                z11_labels.append(y[preds==3])
                z11_count += len(x[preds==3])

        z00_samples = np.concatenate(z00_samples)[:config['sample_num_group_npz']]
        z00_labels = np.concatenate(z00_labels)[:config['sample_num_group_npz']]

        z01_samples = np.concatenate(z01_samples)[:config['sample_num_group_npz']]
        z01_labels = np.concatenate(z01_labels)[:config['sample_num_group_npz']]

        z10_samples = np.concatenate(z10_samples)[:config['sample_num_group_npz']]
        z10_labels = np.concatenate(z10_labels)[:config['sample_num_group_npz']]

        z11_samples = np.concatenate(z11_samples)[:config['sample_num_group_npz']]
        z11_labels = np.concatenate(z11_labels)[:config['sample_num_group_npz']]
    ...
    z00_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    z01_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    z10_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    z11_FID_db = np.zeros(config['sample_num_sets']) # modified by sum
    ...
    print('z00_FID_db =', z00_FID_db) # modified by sum
    print('z01_FID_db =', z01_FID_db) # modified by sum
    print('z10_FID_db =', z10_FID_db) # modified by sum
    print('z11_FID_db =', z11_FID_db) # modified by sum
    print('fairness_db =', l2_db)
    
    test_log.log(epoch=int(state_dict['epoch']), itr=int(state_dict['itr']),
                 FID=float(FID_db.mean().item()), 
                 Z00_FID=float(z00_FID_db.mean().item()),
                 Z01_FID=float(z01_FID_db.mean().item()), 
                 Z10_FID=float(z10_FID_db.mean().item()),
                 Z11_FID=float(z11_FID_db.mean().item()),
