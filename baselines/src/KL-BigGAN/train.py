...
def run(config):
    ...
    if not config['multi']:
        ...
        state_dict = {'itr': 0, 'epoch': 0, 'save_num': 0, 'save_best_num_fair': 0, 'save_best_num_fid': 0, 'save_best_num_tradeoff': 0,
                      ...
                      'best_intra_tradeoff': 999999, 'config': config}
    ...
    if not config['mixed_training']:
        loaders = utils.get_data_loaders(
            config, **{**config, 'batch_size': D_batch_size,
            'start_itr': state_dict['itr']})
    else:
        biased_loaders = utils.get_biased_data_loaders(
            config, **{**config, 'batch_size': D_batch_size,
            'start_itr': state_dict['itr']})
        ref_loaders = utils.get_ref_data_loaders(
            config, **{**config, 'batch_size': D_batch_size,
            'start_itr': state_dict['itr']})
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        ...
        if not config['mixed_training']:
            ...
            if epoch < config['epoch_ref_start']:
                if config['pbar'] == 'mine':
                    pbar = utils.progress(
                        biased_loaders[0], displaytype='s1k' if config['use_multiepoch_sampler'] else 'eta')
                else:
                    pbar = tqdm(biased_loaders[0])
            else:
                if config['fine_tuning']:
                    G.optim.param_groups[0]['lr'] = config['G_lr'] * config['tuning_rate_G']
                    D.optim.param_groups[0]['lr'] = config['D_lr'] * config['tuning_rate_D']
                if config['pbar'] == 'mine':
                    pbar = utils.progress(
                        ref_loaders[0], displaytype='s1k' if config['use_multiepoch_sampler'] else 'eta')
                else:
                    pbar = tqdm(ref_loaders[0])
        ...
        if (epoch >= config['start_eval']):
            ...
            if config['intra_FID']:
                if config['multi']:
                    ...
                    train_fns.update_intra_FID_multi(G, D, G_ema, state_dict, config, FID, z00_FID, z01_FID, z10_FID, z11_FID,
                    ...
                    z0_FID = fid_score.calculate_fid_given_paths([z0_data_moments, z0_sample_moments],
                                                                 batch_size=100, cuda=True, dims=2048)
                    print("FID calculated")
                    train_fns.update_intra_FID(G, D, G_ema, state_dict, config, FID, z1_FID, z0_FID,
                                               experiment_name, test_log, epoch, writer)  # added epoch logging
                    writer.flush()
                ...
                print("FID calculated")
                train_fns.update_FID(G, D, G_ema, state_dict, config, FID, experiment_name, test_log, epoch, writer)  # added epoch logging
                writer.flush()
