    ...
    def train(x, y, ratio):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                if (config['loss_type'] == 'hinge_dis_linear_gen') or (config['loss_type'] == 'hinge_dis_hinge_gen'):
                    ...
                    gradient_penalty = compute_gradient_penalty(GD, z_[:config['batch_size']],
                                                                y_[:config['batch_size']], x[counter], y_counter, config)
                    D_loss = (D_loss_real + D_loss_fake + config['lambda_gp'] * gradient_penalty) / \
                        float(config['num_D_accumulations'])
...
''' This function runs the inception metrics code, checks if the results
    ...
    improvement. '''
def update_FID(G, D, G_ema, state_dict, config, FID, experiment_name, test_log, epoch, writer):
    if not config['multi']:
    ...
    # save model by both best FID and fairness discrepancy
...
def update_intra_FID(G, D, G_ema, state_dict, config, FID, z1_FID, z0_FID, experiment_name, test_log, epoch, writer):
    print('Pre-loading pre-trained single-attribute classifier...')
    if config['dataset'] == 'CA64':
        clf_state_dict = torch.load(CLF_PATH_celeba)['state_dict']
    elif config['dataset'] == 'UTKFace':
...
def update_intra_FID_multi(G, D, G_ema, state_dict, config, FID, z00_FID, z01_FID, z10_FID, z11_FID,
