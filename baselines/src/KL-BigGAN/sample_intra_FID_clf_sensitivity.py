...
POOR_CLF_PATH_celeba = '../src/results/celeba/attr_clf_poor/model_best.pth.tar'
...
def run(config):
    ...
    state_dict = {'itr': 0, 'epoch': 0, 'save_num': 0, 'save_best_num_fair': 0, 'save_best_num_fid': 0, 'best_IS': 0, 'best_FID': 999999,
                  'best_fair_d': 999999, 'config': config}
    ...
    print('Pre-loading pre-trained single-attribute classifier...')
    if config['dataset'] == 'CA64':
        poor_clf_state_dict = torch.load(POOR_CLF_PATH_celeba)['state_dict']
    elif config['dataset'] == 'UTKFace':
        poor_clf_state_dict = torch.load(CLF_PATH_UTKFace)['state_dict']
    elif config['dataset'] == 'FairFace':
        poor_clf_state_dict = torch.load(CLF_PATH_FairFace)['state_dict']
    clf_classes = 2
        
    poor_clf = ResNet18(block=BasicBlock, layers=[2, 2, 2, 2],
                        num_classes=clf_classes, grayscale=False) 
    poor_clf.load_state_dict(poor_clf_state_dict)
    poor_clf = poor_clf.to(device)
    poor_clf.eval()  # turn off batch norm
    ...
    utils.load_weights(G if not (config['use_ema']) else None, None, state_dict, weights_root,
                       experiment_name, config['load_weights'], G if config['ema'] and config['use_ema'] else None,
                       strict=False, load_optim=False)
    ...
    for k in range(10):
        ...
        print('For computing the sensitivity of intra-FID, distributing images for each group and saving them to npz...')
        ...
        z1_samples = np.concatenate(z1_samples)
        z1_labels = np.concatenate(z1_labels)

        z0_samples = np.concatenate(z0_samples)
        z0_labels = np.concatenate(z0_labels)
        ...
        z1_npz_filename = '%s/%s/%s_%s_%s_z1_samples_%s.npz' % (
            config['samples_root'], experiment_name, config['mode'],
            'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', k)
        
        z0_npz_filename = '%s/%s/%s_%s_%s_z0_samples_%s.npz' % (
            config['samples_root'], experiment_name, config['mode'],
            'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', k)
        ...
        z1_npz_filename_poor = '%s/%s/%s_%s_%s_z1_samples_poor_%s.npz' % (
            config['samples_root'], experiment_name, config['mode'],
            'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', k)
        
        z0_npz_filename_poor = '%s/%s/%s_%s_%s_z0_samples_poor_%s.npz' % (
            config['samples_root'], experiment_name, config['mode'],
            'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', k)
        ...
        print('Saving z1 npz to %s...' % z1_npz_filename)
        print('Saving z0 npz to %s...' % z0_npz_filename)
        np.savez(z1_npz_filename_poor, **{'x': z1_samples, 'y': z1_labels})
        np.savez(z0_npz_filename_poor, **{'x': z0_samples, 'y': z0_labels})
    ...
    z1_FID_db = np.zeros(10) # modified by sum
    z0_FID_db = np.zeros(10) # modified by sum
    z1_FID_poor_db = np.zeros(10) # modified by sum
    z0_FID_poor_db = np.zeros(10) # modified by sum
    ...
    if config['dataset'] == 'CA64':
        ...
        unbiased_data = '../fid_stats/celeba/unbiased_all_gender_fid_stats.npz'
        z1_data = '../fid_stats/celeba/male_fid_stats.npz'
        z0_data = '../fid_stats/celeba/female_fid_stats.npz'
        
    elif config['dataset'] == 'UTKFace':
        unbiased_data = '../fid_stats/UTKFace/unbiased_all_race_fid_stats.npz'
        z1_data = '../fid_stats/UTKFace/non_white_fid_stats.npz'
        z0_data = '../fid_stats/UTKFace/white_fid_stats.npz'
        
    elif config['dataset'] == 'FairFace':
        unbiased_data = '../fid_stats/FairFace/unbiased_all_race_fid_stats.npz'
        z1_data = '../fid_stats/FairFace/black_fid_stats.npz'
        z0_data = '../fid_stats/FairFace/white_fid_stats.npz'
        
    # number of classes
    probs_db = np.zeros((10, 100000, n_classes))
    for i in range(10):
        ...
        z1_npz_filename = '{}/{}/{}_{}_{}_z1_samples_{}.npz'.format(
            config['samples_root'], experiment_name, config['mode'], 'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', i)
        z0_npz_filename = '{}/{}/{}_{}_{}_z0_samples_{}.npz'.format(
            config['samples_root'], experiment_name, config['mode'], 'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', i)
        z1_npz_filename_poor = '{}/{}/{}_{}_{}_z1_samples_poor_{}.npz'.format(
            config['samples_root'], experiment_name, config['mode'], 'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', i)
        z0_npz_filename_poor = '{}/{}/{}_{}_{}_z0_samples_poor_{}.npz'.format(
            config['samples_root'], experiment_name, config['mode'], 'ema' if config['use_ema'] else '',
            'G_eval' if config['G_eval_mode'] else 'G_train', i)
        ...
        z1_FID = fid_score.calculate_fid_given_paths([z1_data, z1_npz_filename], batch_size=100, cuda=True, dims=2048)
        z0_FID = fid_score.calculate_fid_given_paths([z0_data, z0_npz_filename], batch_size=100, cuda=True, dims=2048)
        z1_FID_poor = fid_score.calculate_fid_given_paths([z1_data, z1_npz_filename_poor], batch_size=100, cuda=True, dims=2048)
        z0_FID_poor = fid_score.calculate_fid_given_paths([z0_data, z0_npz_filename_poor], batch_size=100, cuda=True, dims=2048)
        ...
        z1_FID_db[i] = z1_FID # modified by sum
        z0_FID_db[i] = z0_FID # modified by sum
        z1_FID_poor_db[i] = z1_FID_poor # modified by sum
        z0_FID_poor_db[i] = z0_FID_poor # modified by sum
        probs_db[i] = probs
        print('fair_disc and FID for iter {} is: l2:{}, l1:{}, kl:{}, FID: {}, z1_FID: {}, z0_FID: {}, z1_FID_poor: {}, z0_FID_poor: {}'
              .format(i, l2, l1, kl, FID, z1_FID, z0_FID, z1_FID_poor, z0_FID_poor)) # modified by sum
    metrics['l2'] = l2_db
    metrics['l1'] = l1_db
    metrics['kl'] = kl_db
    metrics['FID'] = FID_db # modified by sum
    metrics['z1_FID'] = z1_FID_db # modified by sum
    metrics['z0_FID'] = z0_FID_db # modified by sum
    metrics['z1_FID_poor'] = z1_FID_poor_db # modified by sum
    metrics['z0_FID_poor'] = z0_FID_poor_db # modified by sum
    print('Fairness Discrepancies and FID saved in {}'.format(fname)) # modified by sum
    print(FID_db) # modified by sum
    print(z1_FID_db) # modified by sum
    print(z0_FID_db) # modified by sum
    print(z1_FID_poor_db) # modified by sum
    print(z0_FID_poor_db) # modified by sum
    print(l2_db)
